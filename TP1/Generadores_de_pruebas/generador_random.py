...
MULTIPLICADOR_RANGO = 3

def generar_listas(volumen, ruta):
    ...
    try:
        with open(TMP_INTERVALOS, ESCRITURA, encoding=ENCODING) as intervalos, open(TMP_TIMESTAMPS, "w", encoding=ENCODING) as timestamps:
            ...
            for _ in range(volumen):
                t = random.randint(1, volumen * MULTIPLICADOR_RANGO)
                e = random.randint(1, t)
                s.append(random.randint(t - e, t + e))
                intervalos.write(f'{t},{e}\n')

            s.sort()
            for timestamp in s:
                timestamps.write(f'{timestamp}\n')
...
def main():
    ...
    generar_listas(args.volumen, args.ruta)
